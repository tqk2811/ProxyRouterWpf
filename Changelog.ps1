...
param(
    ...
    # '' (default) = the whole history, which is right while 1.0 is the only line.
    # Once older lines are no longer interesting, pass e.g. -Since 'v2.0.0'.
    [string]$Since = ''
)
...
if (-not ($Latest -or $Unreleased)) {
    Write-Host "CHANGELOG.md regenerated. Review, then commit it (see Release.ps1)."
}
