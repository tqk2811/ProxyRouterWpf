...
#
# Prerequisites:
#   - you are on 'master' (CI only releases from master),
#   - the tag 'vM.N.0' of the current minor line already exists on origin
#     (git tag vM.N.0; git push origin vM.N.0).
# CI then builds M.N.<commits-since-tag> and attaches the win-x64 zip to the GitHub Release 'vM.N.0'.
...
# 0) CI only releases from master; fail early instead of pushing a commit that never builds.
$branch = "$(git rev-parse --abbrev-ref HEAD)".Trim()
if ($branch -ne 'master') {
    throw "Releases are master-only (current branch: '$branch'). Merge into master first."
}
